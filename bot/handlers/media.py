...
async def on_voice(message: Message, backend: BackendClient) -> None:
    data = await _download_to_bytes(message.bot, message.voice.file_id)  # pyright: ignore[reportOptionalMemberAccess]
    await _send_media(
        message,
        backend,
        data,
...
async def on_audio(message: Message, backend: BackendClient) -> None:
    data = await _download_to_bytes(message.bot, message.audio.file_id)  # pyright: ignore[reportOptionalMemberAccess]
    mime = message.audio.mime_type or "audio/mpeg"  # pyright: ignore[reportOptionalMemberAccess]
    filename = message.audio.file_name or "audio.mp3"  # pyright: ignore[reportOptionalMemberAccess]
    await _send_media(
        message,
        backend,
        data,
...
async def on_document(message: Message, backend: BackendClient) -> None:
    ...
    fname = (doc.file_name or "").lower()  # pyright: ignore[reportOptionalMemberAccess]
    if not fname.endswith(ALLOWED_DOC_EXT):
        await message.answer(
            f"Поддерживаются только {', '.join(ALLOWED_DOC_EXT)}.",
        )
        return
    if (doc.file_size or 0) > MAX_DOC_BYTES:  # pyright: ignore[reportOptionalMemberAccess]
        await message.answer(
            f"Файл слишком большой (>{MAX_DOC_BYTES // 1024 // 1024} МБ)."
        )
        return
    data = await _download_to_bytes(message.bot, doc.file_id)  # pyright: ignore[reportOptionalMemberAccess]
    mime = doc.mime_type or "application/octet-stream"  # pyright: ignore[reportOptionalMemberAccess]
    await _send_media(
        message,
        backend,
        data,
        mime=mime,
        content=message.caption or "",
        filename=doc.file_name or "document.bin",  # pyright: ignore[reportOptionalMemberAccess]
    )
