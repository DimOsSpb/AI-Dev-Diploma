...
class IngestionService:
    ...
    def __init__(self, settings: AppSettings) -> None:
        ...
        self._docstore_path = (
            self._data_dir.parent / f"{settings.rag_collection}_docstore.json"
        )
    ...
    def ingest_files(self, paths: list[str]) -> int:
        ...
        self._persist_docstore()
        logger.info(
            "ingestion: точечно проиндексировано файлов=%d нод=%d",
            len(files),
            len(nodes),
        )
        return len(nodes)
    ...
    def reindex_all(self) -> int:
        ...
        self._pipeline = self._build_pipeline()
        logger.info(
            "ingestion: полная переиндексация коллекции %s",
    ...
    def run_for_file(self, path: Path) -> int:
        ...
        try:
            ...
            logger.info(
                "ingestion: файл проиндексирован file=%s нод=%d", path.name, count
            )
            return count
        except Exception:
