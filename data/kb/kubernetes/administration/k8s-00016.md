...
1. Загрузите последнюю версию с помощью команды:
    ...
    Например, команда загрузки версии  для Linux будет выглядеть следующим образом:
...
1. Загрузите последнюю версию:
    ...
    Например, команда загрузки версии  для macOS будет выглядеть следующим образом:
...
1. Загрузите последнюю версию  по [этой ссылке](https://dl.k8s.io/release/v/bin/windows/amd64/kubectl.exe).

    Либо, если у вас установлен `curl`, выполните команду ниже:

    ```
    curl -LO https://dl.k8s.io/release/v/bin/windows/amd64/kubectl.exe
    ```
